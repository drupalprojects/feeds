...
class FetcherResult implements FetcherResultInterface {
  ...
  protected function checkFile() {
    if (!file_exists($this->filePath)) {
      throw new \RuntimeException(String::format('File %filepath does not exist.', array('%filepath' => $this->filePath)));
    }

    if (!is_readable($this->filePath)) {
      throw new \RuntimeException(String::format('File %filepath is not readable.', array('%filepath' => $this->filePath)));
    }
  }
  ...
  protected function sanitizeFile() {
    ...
    if (!is_writable($this->filePath)) {
      throw new \RuntimeException(String::format('File %filepath is not writable.', array('%filepath' => $this->filePath)));
    }
