...
class AsyncPlugin implements EventSubscriberInterface
{
    ...
    public function onCurlProgress(Event $event)
    {
        if ($event['download_size'] &&
            ($event['upload_size'] || ($event['uploaded'] && $event['handle'] === $event['uploaded']))
        ) {
            // Timeout after 1ms
            curl_setopt($event['download_size'], CURLOPT_TIMEOUT_MS, 1);
            // Even if the response is quick, tell curl not to download the body
            curl_setopt($event['download_size'], CURLOPT_NOBODY, true);
        }
    }
